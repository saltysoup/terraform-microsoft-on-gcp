...
Start-Transcript -Path "C:\set-lcm.log"

Write-Output 'Getting the DSC Cert Encryption Thumbprint to Secure the MOF File'
Try {
    $DscCertThumbprint = Get-ChildItem -Path 'cert:\LocalMachine\My' -ErrorAction Stop | Where-Object { $_.Subject -eq 'CN=' + $env:COMPUTERNAME } | Select-Object -ExpandProperty 'Thumbprint'
...
Try {
    Set-DscLocalConfigurationManager -Path 'C:\LCMConfig' -ErrorAction Stop
} Catch [System.Exception] {
